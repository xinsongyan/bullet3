...
class CubliEnv:
    def __init__(self, urdf_path="/home/xin/codes/bullet3/pybullet/new_examples/model/cubli.urdf",
                 ini_pos=[0, 0, 0.5],
                 ini_quat=[0, 0, 0, 1]):
        # set env
        ...
        self.jointIds = [0, 1, 2]
        p.setJointMotorControlArray(self.robotId, self.jointIds, p.VELOCITY_CONTROL, forces=[0.0]*3, targetVelocities=[0.0]*3)
        p.setJointMotorControlArray(self.robotId, self.jointIds, p.TORQUE_CONTROL, forces=[0.0]*3)
...
if __name__ == "__main__":
    ...
    state = env.reset()
    while True:
        # action = {'torque': p.calculateInverseDynamics(env.robotId, state['q'], state['qd'], [0] * 3)}
        action = {'torque':  [0] * 3}
